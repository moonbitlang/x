...
test {
  ...
  inspect(
    @crypto.bytes_to_hex_string(@crypto.sha1(@encoding.encode(UTF16, input))),
...
test {
  let input = "The quick brown fox jumps over the lazy dog"
  inspect(
    @crypto.bytes_to_hex_string(@crypto.md5(@encoding.encode(UTF16, input))),
    content="b0986ae6ee1eefee8a4a399090126837",
  )

  // buffered
  let ctx = @crypto.MD5::new()
  ctx.update(b"a")
  ctx.update(b"b")
  ctx.update(b"c")
  inspect(
...
test {
  let input = "The quick brown fox jumps over the lazy dog"
  inspect(
    @crypto.bytes_to_hex_string(@crypto.sm3(@encoding.encode(UTF16, input))),
    content="fc2b31896629e88652ca1e3be449ec7ec93f7e5e29769f273fb973bc1858c66d",
  )

  //buffered
  let ctx = @crypto.SM3::new()
...
test {
  inspect(
    @crypto.bytes_to_hex_string(@crypto.md4(b"abc")),
...
test {
  inspect(
    @crypto.bytes_to_hex_string(@crypto.ripemd160(b"abc")),
...
test {
  inspect(
    @crypto.bytes_to_hex_string(@crypto.sha512(b"abc")),
  ...
  inspect(
    @crypto.bytes_to_hex_string(@crypto.sha384(b"abc")),
...
test {
  ...
  let encrypted = @crypto.aes_ecb_encrypt(key, data)
  inspect(
